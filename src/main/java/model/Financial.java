package model;

import java.sql.*;

public class Financial {
	//A common method to connect to the DB
	private Connection connect(){
	 Connection con = null;
	 try
	 {
		 Class.forName("com.mysql.jdbc.Driver");

		 //Provide the correct details: DBServer/DBName, username, password
		 con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3307/financial", "root", "");
	 }
	 catch (Exception e)
	 {e.printStackTrace();}
	 
	 return con;
	 } 
	
	
	
	
	
	
	
	public String insertBill(String BillDate, String ConsumedUnits, String UnitPrice, String Total){
	 String output = "";
		try{
			Connection con = connect();
			if (con == null){
				return "Error while connecting to the database for inserting.";
				}
			
			
	 // create a prepared statement
	 String query = " insert into financial (`BillID`,`BillDate`,`ConsumedUnits`,`UnitPrice`,`Total`)"+ " values (?, ?, ?, ?, ?)";
	 PreparedStatement preparedStmt = con.prepareStatement(query);
	 // binding values
	 	preparedStmt.setInt(1, 0);
	 	preparedStmt.setString(2, BillDate);
	 	preparedStmt.setInt(3, Integer.parseInt(ConsumedUnits));
	 	preparedStmt.setDouble(4, Double.parseDouble(UnitPrice));
	 	preparedStmt.setDouble(5, Double.parseDouble(Total));
	 // execute the statement
	 
	 preparedStmt.execute();
	 	con.close();
	 	output = "Inserted successfully";
	 }
	 catch (Exception e){
		 output = "Error while inserting to the Database.";
		 System.err.println(e.getMessage());
	 }
		return output;
	 }
	
	
	
	
	
	public String readBills(){
	 String output = "";
	 	try{
	 		Connection con = connect();
	 		if (con == null){
	 			return "Error while connecting to the database for reading."; 
	 			}
	 // Prepare the html table to be displayed
	 output = "<table border='1'>"
	 		+ "<tr>"
	 		+ "<th>Bill Date</th>"
	 		+ "<th>Consumed Units</th>" +
	 		"<th>Unit price</th>" +
	 		"<th>Total</th>" +
	 		"<th>Update</th><th>Remove</th></tr>";

	 String query = "select * from financial";
	 Statement stmt = con.createStatement();
	 ResultSet rs = stmt.executeQuery(query);
	 // iterate through the rows in the result set
	 while (rs.next()){
		 String BillID = Integer.toString(rs.getInt("BillID"));
		 String BillDate = rs.getString("BillDate");
		 String ConsumedUnits = Integer.toString(rs.getInt("ConsumedUnits"));
		 String UnitPrice = Double.toString(rs.getDouble("UnitPrice"));
		 String Total = Double.toString(rs.getDouble("Total"));
	 // Add into the html table
		 output += "<tr><td>" + BillDate + "</td>";
		 output += "<td>" + ConsumedUnits + "</td>";
		 output += "<td>" + UnitPrice + "</td>";
		 output += "<td>" + Total + "</td>";
	
		 output += "<td><input name='btnUpdate' type='button' value='Update' class='btn btn-secondary'></td>"
				 + "<td><form method='post' action='items.jsp'>"
				 + "<input name='btnRemove' type='submit' value='Remove' class='btn btn-danger'>"
				 + "<input name='BillID' type='hidden' value='" + BillID
				 + "'>" + "</form></td></tr>";
	 }
	 con.close();
	 // Complete the html table
	 output += "</table>";
	 }
	 catch (Exception e){
		 output = "Error while reading the items.";
		 System.err.println(e.getMessage());
	 }
	 	return output;
	 } 
	
	
	

	
	public String updateBills(String BillID, String BillDate, String ConsumedUnits, String UnitPrice, String Total){
		 String output = "";
	
		 try{
			 Connection con = connect();
			 if (con == null){
				 return "Error while connecting to the database for updating."; }
		 // create a prepared statement
			 	String query = "UPDATE financial SET BillDate=?,ConsumedUnits=?,UnitPrice=?,Total=? WHERE BillID=?";
			 	PreparedStatement preparedStmt = con.prepareStatement(query);
		 // binding values
			 	preparedStmt.setString(1, BillDate);
			 	preparedStmt.setInt(2, Integer.parseInt(ConsumedUnits));
			 	preparedStmt.setDouble(3, Double.parseDouble(UnitPrice));
			 	preparedStmt.setDouble(4, Double.parseDouble(Total));
			 	preparedStmt.setInt(5, Integer.parseInt(BillID));
		 // execute the statement
			 	preparedStmt.execute();
			 	con.close();
			 	output = "Updated successfully";
		 	}
		 catch (Exception e){
			 output = "Error while updating the Financial.";
			 System.err.println(e.getMessage());
		 }
		 return output;
		 } 
	
	
	
	
	public String deleteBills(String BillID){
		String output = "";
	 try{
		 Connection con = connect();
		 if (con == null){
			 return "Error while connecting to the database for deleting."; }
	 // create a prepared statement
		 String query = "delete from financial where BillID=?";
		 PreparedStatement preparedStmt = con.prepareStatement(query);
	 // binding values
		 preparedStmt.setInt(1, Integer.parseInt(BillID));
	 // execute the statement
		 preparedStmt.execute();
		 con.close();
		 output = "Deleted successfully";
	 }
	 catch (Exception e){
		 output = "Error while deleting the Financial.";
		 System.err.println(e.getMessage());
	 }
	 	return output;
	 } 
	
	
	
	
	
	

}
